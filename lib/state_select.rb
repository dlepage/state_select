module ActionView::Helpers::FormOptionsHelper
  ...
  def state_options_for_select(selected = nil, country = 'US', full_states=false)
    full = full_states ?  "FULL_" : ""
    return us_canada_select_options(selected) if country == 'US_CANADA'
    state_options = ""
    if country
      state_options += options_for_select(eval("#{country.upcase}_#{full}STATES").map{|state| [state,"#{country}, #{state}"]}, selected) if country
    end
    return state_options
  end
  
  def us_canada_select_options(selected = nil, full_states=false)
      select_option = <<END
      <option value=""></option>
      <optgroup label="US">
        #{state_options_for_select selected, 'US', full_states}
      </optgroup>
      <optgroup label="Canada">
        #{state_options_for_select selected, 'Canada', full_states}
      </optgroup>
END
    end

    def us_canada_select_tag(name, selected = nil, options = {})    
      select_option = us_canada_select_options(selected,options[:full_states])
      select_tag(name, select_option, options)
    end
    def us_canada_select_index(country, state)
      (country == "US" ? US_STATES.index(state) : (CANADA_STATES.index(state) || 0)  + US_STATES.length ) + 1 rescue 0
    end
    
  private
  US_STATES=["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PW","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"] unless const_defined?("US_STATES")  
  US_FULL_STATES=["Alabama", "Alaska", "Arizona", "Arkansas", "California", "Colorado", "Connecticut", "Delaware", "Florida", "Georgia", "Hawaii", "Idaho", "Illinois", "Indiana", "Iowa", "Kansas", "Kentucky", "Louisiana", "Maine", "Maryland", "Massachusetts", "Michigan", "Minnesota", "Mississippi", "Missouri", "Montana", "Nebraska", "Nevada", "New Hampshire", "New Jersey", "New Mexico", "New York", "North Carolina", "North Dakota", "Ohio", "Oklahoma", "Oregon", "Pennsylvania", "Puerto Rico", "Rhode Island", "South Carolina", "South Dakota", "Tennessee", "Texas", "Utah", "Vermont", "Virginia", "Washington", "Washington D.C.", "West Virginia", "Wisconsin", "Wyoming"] unless const_defined?("US_FULL_STATES")
  ...
  CANADA_STATES=["AB", "BC", "MB", "NB", "NL", "NT", "NS", "NU", "ON", "PE", "QC", "SK", "YT"] unless const_defined?("CANADA_STATES")
  CANADA_FULL_STATES=["Alberta", "British Columbia", "Manitoba", "New Brunswick", "Newfoundland", "Northwest Territories", "Nova Scotia", "Nunavut", "Ontario", "Prince Edward Island", "Quebec", "Saskatchewan", "Yukon"] unless const_defined?("CANADA_FULL_STATES")
